...
st.title("Roadmap Delivery Confidence Assessment Generator")

uploaded_excel = st.file_uploader("Upload Excel File", type=["xlsx"])
uploaded_ppt = st.file_uploader("Upload PowerPoint Template", type=["pptx"])
...
if uploaded_excel and uploaded_ppt:
    ...
    match = re.search(r"^(.*?)(\d{1,2}(st|nd|rd|th)?\s+\w+\s+\d{4})", uploaded_excel.name)
    ...
    output = BytesIO()
    prs.save(output)
    st.download_button("Download Presentation", output.getvalue(), file_name="Roadmap Delivery Confidence Assessment - FIXED.pptx")
